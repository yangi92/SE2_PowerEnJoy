...
\begin{document}
	...
	\subsection{Version 1.1 -Updated on 07/01/2017}
	The following changes were made to the previous version 1.0:
\begin{itemize}
\item Class diagram was removed as it is not considered essential in this stage of development.
\item Specific requirements were improved and report action added.
\item New mockups added
...
\end{itemize}
	...
	\section{\Large Introduction}
	 ...
	 \subsection{Purpose}
		...
		requirements of the system, analysing the needs of the customer and explaining common 
		use case scenarios.
	 ...
	 \subsection{\label{scope:1}Scope}
		...
		behaviours of the users with some bonuses:\footnote{Bonuses are not combinabale. Only the highest bonus will be awarded}
	...
	\begin{enumerate}
		...
		\item \textbf{Specifications}: this section gives a deep insight about the system's 
			  ...
			  diagrams . An Alloy model will be provided and discussed and finally the non functional requirements will be discussed.
			  \item \textbf{Appendices} : this section talks about the tools used for the creation of this documents and the time spent working.
	\end{enumerate}	
	\newpage
  ...
  \section{\Large Overall Description}
	...
	\begin{itemize}
		...
		\begin{itemize}
				...
				\item the user must enter a \textbf{valid} payment method \footnote{\label{note1} For 
			    more check out the \hyperref[actors:3]{\textit{Glossary}}}
	    \end{itemize}
	...
	\subsection{Constraints}
	  ...
	  %------------------------------
		...
		A more detailed view about functions and requirements can be found in section 3.
		\hyperref[req:3]	 {\textit{Functional requirements(3.2)}} 
		\begin{enumerate}
		\item \textit{Web and Mobile Application}: both user applications must provide form for user registration and log - in. A user must be able to review his payment history and if necessary update his personal information.
		...
		\item \textit{On-board Application}: must be installed on all cars. It must allow a driver to chose his/her destination and eventually enable the money saving option. The on-board display provides all necessary data to inform the user about the current fee and if the car is located in a safe area. Lastly ,the on-board system must allow the user to end his ride via on-board display.
		\item \textit{Back-End application} : must provide interfaces for registration and login processes checking also if the user input is valid. It must handle correctly search requests and handle the automatic car locking process. Lastly, it must correctly calculate and charge fees based on discounts or penalites.
		\end{enumerate}
 ...
 %-------------------------
 	\section{\Large Specific Requirements}
		...
		\centering
		  ...
		  \caption{Main screen with disabled park car button}
 	 	\end{figure}
 	 	\clearpage
 	 	 	\begin{figure}
		 \centering	
		\includegraphics[scale=0.6]{Images/onBoard/Main2.png}
		 \caption{Main screen with enabled park car button}
		\centering
 	 	\includegraphics[scale=0.6]{Images/onBoard/Park.png}
		  \caption{Car parked main screen}
 	 	\end{figure}
 	 	\clearpage
 	 	 \begin{figure}
		 \centering	
		\includegraphics[scale=0.6]{Images/onBoard/Park2.png}
		 \caption{Charging option screen}
 	 	\end{figure}
 	 	\clearpage
 	 	
 	 	\newpage
 	 
	\subsection{\label{actors:3}Specific Requirements}
	  ...
	  \begin{itemize}
	 		...
	 		\item{\textbf{[R1.9]}}: Allow a registered user to search for an available vehicle 	
	 			 given an input position and a radius.
		...
		\begin{itemize}
 	 		\item{\textbf{[R2.1]}}: Enable a registered user to keep track of the distance 
			...
			\item{\textbf{[R2.3]}}: Provide a method to enable the money saving option.
 	 		\item{\textbf{[R2.3]}}: Enable the driver to end the ride if the car is parked in 
 	 			a safe area.
 	 		\item{\textbf{[R2.4]}}: Enable the driver to charge the car.
 	 	\end{itemize}
 	 	\newpage
 	 	\textbf{3.Back-End System}
		\begin{itemize}
 	 		...
 	 		\item{\textbf{[R3.8]}}:The back-end must provide an interface to allow a support operator to perform
all basics CRUD operations
 	 	...
 	 	\FloatBarrier
 	 	\begin{figure}[htbp]
		\includegraphics[scale=0.3]{Images/UML/GeneralCase.png}
		 \caption{General use case scenrio}
 	 	\end{figure}
 	 	\newpage
	...
	%-----------------------------
	  ...
	  \subsection{Non functional Requirements} 
		...
		\item \emph{Alloy 4.2} for Alloy Modelling
		\item \emph{Git} for version control
		\item \emph{Sketch \& Photoshop} for Mockups
		\item \emph{StarUML} for UML Diagrams
		\end{itemize}
		
		
		\subsection{Hours of work}
		\begin{itemize}
		\item Simone Amico ~ 35 h
		\item Chianella Beatrice ~ 35 h
		\item Giovanakis Yannick ~ 35 h
		\end{itemize}
\end{document}
